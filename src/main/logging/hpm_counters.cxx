...
typedef std::array<unsigned long, NUM_COUNTERS> snapshot_t;
...
static int handle_stats(int enable)
{
   ...
   if (enable == INIT) fp = fopen("statistics.csv", "w");
...
#define READ_CTR(name) do { \
      if (i < NUM_COUNTERS) { \
         unsigned long csr = read_csr_safe(name); \
